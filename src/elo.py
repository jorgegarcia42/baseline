...
def add_elo_and_streaks(
        ...
        yearly_decay: float = 0.20
):
    """
    add pre-match general elo, surface elo and streaks. returns (df, state);
    state can be passed back in to continue elo across years (warmup window).
    ratings decay toward start_elo by yearly_decay per year (flavor a).
    """
    ...
    for _, row in df.iterrows():
        ...
        if current_year is None:
            ...
            for surfaces in surface_elo_dict.values():
                for s in surfaces:
                    surfaces[s] = start_elo + (surfaces[s] - start_elo) * factor
            current_year = row_year
        ...
        # surface elo: pre-match values (seeded on first touch)
